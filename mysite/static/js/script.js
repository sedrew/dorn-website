...
if (device.mobile()) {
  ...
  $(window).scroll(function () {
    ...
    if ($(window).scrollTop() > 360) {
      $(".menu_list").addClass("fixed-top");
      // console.log("Меню должно стать в топе");
    }
    if ($(window).scrollTop() < 350) {
      $(".menu_list").removeClass("fixed-top");
    }
    if ($(window).scrollTop() > 300) {
      if (j < 1) {
        setTimeout(function () {
          $(".donate_mobile").slideDown(100);
        }, 2000);
        j = 1;
        setTimeout(function () {
          $(".donate_mobile").slideUp(100);
        }, 12000)
      }
    }
  })
} else {
  // console.log("Зашли в условие ПК");
  var j = 0;
  $(window).scroll(function () {
